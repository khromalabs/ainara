...
| After a config wipe/restore, protection is quietly weaker | `trading.watchdog.mode` **defaults to `monitor`** (report-only). A wipe that drops the key means the watchdog, on its *next* restart, comes up monitoring but **not auto-closing** a broken hedge — it will alert but not act. The running process keeps whatever mode it started with, so the downgrade is invisible until a restart. | After restoring config, confirm `trading.watchdog: mode: active` is present (the recovery snapshot from `config-backups` has it). The watchdog's startup Discord push and log also state the mode — check them after any restart during an incident. |
