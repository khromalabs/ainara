...
2.  **"reinforce"**: If the conversation confirms, restates, or adds new details to an existing memory.
    ...
    - **If the memory text can to be updated** as an improvement, with an SMALL AMOUNT of new information directly related with the memory content, also provide the `new_memory_text` which synthesizes the old memory with the new details. If the amount of information to be added to a memory is larger than simple phrase or a few terms OR if the updated memory is becomes larger than 100 words, create a new memory instead.
